...
public class SwfImagesExporter {
    ...
    public void Export(string swfFolderPath) {
        ImageData[] imageDatas = GetImageDatas(true);
    ...
    /// <returns></returns>
    private ImageData[] GetImageDatas(bool isOnlyExportLinkage) {
        // 获取有链接类名的 DefineSprite(在SymbolClassTag中定义) 的所有 CharacterId

        var imageDatas = new List<ImageData>();
        for (int i = 0, len = m_swf.tags.Count; i < len; i++) {
            var tag = m_swf.tags[i];
            if (tag.header.type == (uint)TagType.DefineBits) {
                var defineBitsTag = (DefineBitsTag)tag;
                if (isOnlyExportLinkage && m_swf.linkageDefineCharacterIds.IndexOf(defineBitsTag.characterID) > -1) {
                ...
                if (isOnlyExportLinkage && m_swf.linkageDefineCharacterIds.IndexOf(defineBitsJpeg2Tag.characterID) > -1) {
                ...
                if (isOnlyExportLinkage && m_swf.linkageDefineCharacterIds.IndexOf(defineBitsJpeg3Tag.characterID) > -1) {
                ...
                if (isOnlyExportLinkage && m_swf.linkageDefineCharacterIds.IndexOf(defineBitsLosslessTag.characterID) > -1) {
                ...
                if (isOnlyExportLinkage && m_swf.linkageDefineCharacterIds.IndexOf(defineBitsLossless2Tag.characterID) > -1) {
                ...
                if (isOnlyExportLinkage && m_swf.linkageDefineCharacterIds.IndexOf(defineBitsJpeg4Tag.characterID) > -1) {
